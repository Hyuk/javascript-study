...
// filtering director -> extract the arrays that the director is Christopher Nolan
// map the imdbratings -> [8.8, 8.6, 9, 8.3]
// reduce to add the numbers
// devide by number 4
...
function splitify(str) {
  return str.split(/\W/); // W is Capital letter
}
splitify("Hello World,I-am code");
...
function urlSlug(title) {
  let localTitle = title.toLowerCase().trim().split(/\s+/).join("-");
  return localTitle;
}
...
```javascript
// check the every element in array is all positive.
...
```javascript
// check the some element in array is positive.
function checkPositive(arr) {
  // Add your code below this line
