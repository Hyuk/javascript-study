...
function add (arr, bookName) {
  let newArr = [...arr]; // [...arr] : 로컬 변수 newArr Array에 함수 첫번째 인수로 받은 arr Array를 복사한다.
  newArr.push(bookName); // push : 함수 두번째 인수로 받은 bookName를 newArr Array의 마지막 인덱스에 추가한다.
  return newArr;
}
...
// WatchList Arrpay의 각 인덱스를 맵핑해서 원하는 값만을 rating Array 변수에 저장한다.
